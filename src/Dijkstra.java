...
public class Dijkstra {
    ...
    public void compute(Vertex source){
        ...
        source.setDistance(0);// присваиваем первой вершине расстояние "0"
        ...
        while (!queue.isEmpty()){
            ...
            // проходим по всем соседям, а узнаем соседей через ребра
            for (Edge e : curr.getNeighbors()){
                ...
                // если еще не был в вершине в конце ребра
                if (!end.isVisited()){
                    // если сумма нынешней вершины и вес ребра меньше чем значение вершины в конце ребра
                    if (curr.getDistance() + w < end.getDistance()){
                        // меняем значение вершины в конце ребра на расстояние покороче
